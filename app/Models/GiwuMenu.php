...
class GiwuMenu extends Model {
	...
	protected $table = 'dnpmsiteg_menu';
	...
	public static function getListMenuTopIdEgaVari($idTopMenu){
        ...
        return GiwuRoleAcces::join('dnpmsiteg_menu','dnpmsiteg_menu.id_menu','dnpmsiteg_role_acces.id_menu')
                            ->where('dnpmsiteg_role_acces.role_id',Auth::user()->id_role)
                            ->where('dnpmsiteg_role_acces.statut_role',1)
                            ->where('dnpmsiteg_menu.topmenu_id',$idTopMenu)
                            ->where('dnpmsiteg_menu.elmt_menu',"oui")
                            ->whereNotIn('dnpmsiteg_menu.id_menu',[1])
                            ->orderby('dnpmsiteg_menu.num_ordre','asc')
                            ->get();
    }
