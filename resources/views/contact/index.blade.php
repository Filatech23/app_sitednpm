...
@section('content')
	...
	<div class="col-lg-12">
		<div class="card"  id="ticketsList">
			<div class="card-header border-0">
				<div class="d-flex align-items-center"><h5 class="card-title mb-0 flex-grow-1"><i class="{{$icone}} m-2"></i>{{$titre}}</h5>
					<div class="flex-shrink-0">
						...
						@endif
						@if(in_array('exporter_contact',session('InfosAction')))
				...
				{!! Form::open(array('id' => 'formSearch','class' => '', 'method' => 'GET')) !!}
					<div class="row gy-4">
						...
						<div class="col-xxl-3 col-md-4" > <!--Code en commentaire a remplacer display:none si l'element est parametre -->
							<?php // echo ((!in_array('combo_contact,session('InfosAction'))) ? "style='display:none;'" : '')?>
							<div><label for="labelInput" class="form-label">Liste des Status</label>
								{!! Form::select('statut_cont',trans('entite.traite'),session('statut_contSess'),["id"=>"statut_cont","onchange"=>"funcRecher()","class"=>"form-select allselect"]) !!}
							</div>
						</div>
			...
			<div class="card-body"><div id='dataRefresh' class="table-responsive table-card mb-4 m-2 giwuRefresh">@include('contact.index-search')</div></div>
		</div><!--end card-->
	</div><!--end col-->
...
@section('JS_content')
	...
	<script type="text/javascript">
		$(document).ready(function(){
			$(".exporterXls").attr('href','{{url("contact/exporterExcel")}}');
			$(".exporterPdf").attr('href','{{url("contact/exporterPdf")}}');
			// Pagination
			...
			function fetch_page(page){
				...
				$.ajax({ url:"contact?page="+page, data: dr, type: 'GET',
		...
		function funcRecher(){
			...
			$(".exporterXls").attr('href','{{url("contact/exporterExcel")}}?'+filtreData);
			$(".exporterPdf").attr('href','{{url("contact/exporterPdf")}}?'+filtreData);
			...
			return $.ajax({
				url: '{{ url("/contact/")}}',data: filtreData,type: 'GET',
		...
		$(document).on('click', '.btn-add', function () {
			$.ajax({url : 'contact/create',type : 'GET',dataType : 'html',
		...
		$(document).on('click', '#btn-modifier', function () {
			...
			if(id == ''){
				...
				$.ajax({url : 'contact/'+id+'/edit',type : 'GET',dataType : 'html',
		...
		$(document).on('click', '#btn-supprimer', function () {
			...
			if(id == ''){
				...
				$.ajax({url : 'contact/'+id,type : 'GET',dataType : 'html',
