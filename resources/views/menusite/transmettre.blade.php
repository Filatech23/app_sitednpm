	...
	<div class="modal-body text-center p-4">
		<div class="mt-2">
			@if($type == 'trans') <!-- Code Transmettre -->
				...
				<p class='text-muted mb-4'> Voulez-vous vraiment transmettre {{$item->libelle_menu}} pour validation ? </p>
				<form  action="{{ url('/menusite/affect'.$type.'/'.$item->id_menusite)}}" method='POST'>
					@csrf()
			...
			@elseif($type == 'go') <!-- Code Validation -->
				...
				<p class='text-muted mb-4'> Voulez-vous vraiment publier {{$item->libelle_menu}} ? </p>
				<form  action="{{ url('/menusite/affect'.$type.'/'.$item->id_menusite)}}" method='POST'>
					@csrf()
					<button type="button" class="btn btn-light" data-bs-dismiss="modal">Non</button>
			...
			@elseif($type == 'stop') <!-- Code Arreter de publier -->
				...
				<p class='text-muted mb-4'> Voulez-vous vraiment arr&ecirc;ter de publier {{$item->libelle_menu}} ? </p>
				<form  action="{{ url('/menusite/affect'.$type.'/'.$item->id_menusite)}}" method='POST'>
					@csrf()
					<button type="button" class="btn btn-light" data-bs-dismiss="modal">Non</button>
