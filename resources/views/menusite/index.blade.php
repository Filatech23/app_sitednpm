...
@section('content')
	...
	<div class="col-lg-12">
		<div class="card"  id="ticketsList">
			<div class="card-header border-0">
				<div class="d-flex align-items-center"><h5 class="card-title mb-0 flex-grow-1"><i class="{{$icone}} m-2"></i>{{$titre}}</h5>
					<div class="flex-shrink-0">
						...
						@endif
						@if(in_array('exporter_menusite',session('InfosAction')))
				...
				{!! Form::open(array('id' => 'formSearch','class' => '', 'method' => 'GET')) !!}
					<div class="row gy-4">
						...
						<div class="col-xxl-3 col-md-4" >
							<div><label for="labelInput" class="form-label">Liste des Parents</label>
								<?php $addUse = array('-1'=>'Liste des menus principales'); $listid_parent = $addUse + $listid_parent->toArray();?>
								{!! Form::select('id_parent',$listid_parent ,session('id_parentSess'),["id"=>"id_parent","onchange"=>"funcRecher()","class"=>"form-select allselect"]) !!}
							</div>
						</div>
			...
			<div class="card-body"><div id='dataRefresh' class="table-responsive table-card mb-4 m-2 giwuRefresh">@include('menusite.index-search')</div></div>
		</div><!--end card-->
	</div><!--end col-->
...
@section('JS_content')
	...
	<script type="text/javascript">
		$(document).ready(function(){
			$(".exporterXls").attr('href','{{url("menusite/exporterExcel")}}');
			$(".exporterPdf").attr('href','{{url("menusite/exporterPdf")}}');
			// Pagination
			...
			function fetch_page(page){
				...
				$.ajax({ url:"menusite?page="+page, data: dr, type: 'GET',
		...
		function funcRecher(){
			...
			$(".exporterXls").attr('href','{{url("menusite/exporterExcel")}}?'+filtreData);
			$(".exporterPdf").attr('href','{{url("menusite/exporterPdf")}}?'+filtreData);
			...
			return $.ajax({
				url: '{{ url("/menusite/")}}',data: filtreData,type: 'GET',
		...
		$(document).on('click', '.checkradio', function () { 
			...
			if($(this).data('etat_menu') != 'e'){$('#btn-transmettre').addClass('d-none');}else{$('#btn-transmettre').removeClass('d-none');}
			if($(this).data('etat_menu') == 't'){$('#btn-go').removeClass('d-none');$('#btn-stop').addClass('d-none');$('#btn-return').removeClass('d-none');
			}else if($(this).data('etat_menu') == 'p'){$('#btn-go').addClass('d-none');$('#btn-return').addClass('d-none');$('#btn-stop').removeClass('d-none');}
		});

		
		$(document).on('click', '.btn-add', function () {
			$.ajax({url : 'menusite/create',type : 'GET',dataType : 'html',
		...
		$(document).on('click', '#btn-modifier', function () {
			...
			if(id == ''){
				...
				$.ajax({url : 'menusite/'+id+'/edit',type : 'GET',dataType : 'html',
		...
		$(document).on('click', '#btn-supprimer', function () {
			...
			if(id == ''){
				...
				$.ajax({url : 'menusite/'+id,type : 'GET',dataType : 'html',
		...
		$(document).on('click', '#btn-return', function () {
			...
			if(id == ''){
				...
				$.ajax({url : '{{ url("menusite/AffichePopAction/") }}/'+id,type : 'GET',dataType : 'html',
		...
		$(document).on('click', '#btn-go,#btn-stop,#btn-transmettre', function () {
			...
			if(id == ''){
				...
				$.ajax({url : 'menusite/valide'+typ+'/'+id,type : 'GET',dataType : 'html',
					success : function(code_html, statut){$('#kt_delete_4 .modal-dialog').html(code_html);$('#kt_delete_4').modal('show');}
				});
			}
		});
