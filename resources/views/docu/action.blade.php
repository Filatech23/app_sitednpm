...
<div class="modal-content">
	...
	<div class="modal-body">
		...
		<form id="formAction" class="needs-validation" novalidate>
			{!! Form::hidden('id_doc',$item->id_doc,["class"=>"form-control"]) !!}
			@csrf()
			<div>
				<div class="mb-12">
					<label for="motif_doc" class="form-label">{!!trans('data.motif_doc')!!} </label>
					{!! Form::textarea('motif_doc',$item->motif_doc,["id"=>"motif_doc","class"=>"form-control" ,'autocomplete'=>'off' ,'placeholder'=>"Entrer Motif" , 'rows'=>'4'  ]) !!}
					<span class="text-danger" id="motif_docError"></span>
				</div>
			</div>
...
<script type="text/javascript">
	function addAction(){
		...
		$('#motif_docError').addClass('d-none');
		$.ajax({
			type: 'POST',url: '{{ url("/docu/actionUpdate/")}}',data: $('#formAction').serialize(),
