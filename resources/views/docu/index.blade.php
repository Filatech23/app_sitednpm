...
@section('content')
	...
	<div class="col-lg-12">
		<div class="card"  id="ticketsList">
			<div class="card-header border-0">
				<div class="d-flex align-items-center"><h5 class="card-title mb-0 flex-grow-1"><i class="{{$icone}} m-2"></i>{{$titre}}</h5>
					<div class="flex-shrink-0">
						...
						@endif
						@if(in_array('exporter_document',session('InfosAction')))
				...
				{!! Form::open(array('id' => 'formSearch','class' => '', 'method' => 'GET')) !!}
					<div class="row gy-4">
						...
						<div class="col-xxl-3 col-md-4" >
							<div><label for="labelInput"  title="Menus dont les types sont égaux à document" class="form-label">Liste des Menus de type document</label>
								<?php $addUse = array(''=>'Sélectionnez un élément'); $listmenusite_id = $addUse + $listmenusite_id->toArray();?>
								{!! Form::select('menusite_id',$listmenusite_id ,session('menusite_idSess'),["id"=>"menusite_id","onchange"=>"funcRecher()","class"=>"form-select allselect"]) !!}
							</div>
						</div>
			...
			<div class="card-body"><div id='dataRefresh' class="table-responsive table-card mb-4 m-2 giwuRefresh">@include('docu.index-search')</div></div>
		</div><!--end card-->
	</div><!--end col-->
...
@section('JS_content')
	...
	<script type="text/javascript">
		$(document).ready(function(){
			$(".exporterXls").attr('href','{{url("docu/exporterExcel")}}');
			$(".exporterPdf").attr('href','{{url("docu/exporterPdf")}}');
			// Pagination
			...
			function fetch_page(page){
				...
				$.ajax({ url:"docu?page="+page, data: dr, type: 'GET',
		...
		function funcRecher(){
			...
			$(".exporterXls").attr('href','{{url("docu/exporterExcel")}}?'+filtreData);
			$(".exporterPdf").attr('href','{{url("docu/exporterPdf")}}?'+filtreData);
			...
			return $.ajax({
				url: '{{ url("/docu/")}}',data: filtreData,type: 'GET',
		...
		$(document).on('click', '.checkradio', function () { 
			...
			if($(this).data('etat_doc') != 'e'){$('#btn-transmettre').addClass('d-none');}else{$('#btn-transmettre').removeClass('d-none');}
			if($(this).data('etat_doc') == 't'){$('#btn-go').removeClass('d-none');$('#btn-stop').addClass('d-none');$('#btn-return').removeClass('d-none');
			}else if($(this).data('etat_doc') == 'p'){$('#btn-go').addClass('d-none');$('#btn-return').addClass('d-none');$('#btn-stop').removeClass('d-none');}
		});

		
		$(document).on('click', '.btn-add', function () {
			$.ajax({url : 'docu/create',type : 'GET',dataType : 'html',
		...
		$(document).on('click', '#btn-modifier', function () {
			...
			if(id == ''){
				...
				$.ajax({url : 'docu/'+id+'/edit',type : 'GET',dataType : 'html',
		...
		$(document).on('click', '#btn-supprimer', function () {
			...
			if(id == ''){
				...
				$.ajax({url : 'docu/'+id,type : 'GET',dataType : 'html',
		...
		$(document).on('click', '#btn-return', function () {
			...
			if(id == ''){
				...
				$.ajax({url : '{{ url("docu/AffichePopAction/") }}/'+id,type : 'GET',dataType : 'html',
		...
		$(document).on('click', '#btn-go,#btn-stop,#btn-transmettre', function () {
			...
			if(id == ''){
				...
				$.ajax({url : 'docu/valide'+typ+'/'+id,type : 'GET',dataType : 'html',
					success : function(code_html, statut){$('#kt_delete_4 .modal-dialog').html(code_html);$('#kt_delete_4').modal('show');}
				});
			}
		});
