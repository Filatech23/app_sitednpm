        ...
        <div class="card">
            ...
            <div class="card-body">
                ...
                <strong><div class="msgAjouter"></div></strong>
                    <form action="{{route('role.update',$item->id_role)}}" method="post" id="form" class="row g-3 needs-validation" novalidate  enctype="multipart/form-data">
                        @csrf()
                        @method('PATCH')
                        <div class="row">
                            ...
                            <div class="col-md-12">
                                <i>
                                    NB : Il est formellement interdire de donner à un même rôle l'action de "Valider contenu" et "Transmettre contenu". Soite l'un ou l'autre. Merci. 
                                </i>
                            </div><br><br>
                            <div class="col-md-12">
                                <div class="mb-3">
                                    <label for="libelle_role" class="form-label">{{trans('data.libelle_role')}}<strong style='color: red;'> *</strong></label>
                                    {!! Form::text('libelle_role',$item->libelle_role,["id"=>"libelle_role","class"=>"form-control","required"=>"required",'placeholder'=>"Entrer le role",'autocomplete'=>'off']) !!}
                                </div>
                            </div>
                            <div class="col-md-12">
                                            ...
                                            <tr role="row">
                                                <td>                                                 
                                                    <div class="form-check form-switch">
                                                        <input class="form-check-input" {{$checkMenu}} type="checkbox" role="switch" id="flexSwitchCheckChecked{{$list->id_menu}}" name ="cocher{{$list->id_menu}}" value="{{$list->id_menu}}">
                                                ...
                                                @foreach($cheActio as $listAct)
                                                    ...
                                                    <tr role="row" style="color:#61a7a3;">
                                                        <td style="padding-left:70px;">
                                                            <div class="form-check form-switch">
                                                                <input class="form-check-input" {{$checkAction}} type="checkbox" role="switch" name ="action{{$listAct->id_action}}"  value="{{$listAct->id_action}}" id="flexSwitchCheck{{$listAct->id_action}}" >
